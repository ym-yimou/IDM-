...
::  要激活，请使用 "/act" 参数运行脚本或将下面一行中的 0 改为 1
set _activate=0

::  要冻结30天试用期，请使用 "/frz" 参数运行脚本或将下面一行中的 0 改为 1
set _freeze=0

::  要重置激活和试用，请使用 "/res" 参数运行脚本或将下面一行中的 0 改为 1
set _reset=0

::  如果更改了上述行中的值或使用了参数，则脚本将以无人值守模式运行

::========================================================================================================================================

::  设置路径变量，如果系统配置错误，此操作会有所帮助
...
::  如果在x64位Windows上由x86进程启动，则使用x64进程重新启动脚本
::  如果在ARM64 Windows上由x86/ARM32进程启动，则使用ARM64进程重新启动脚本
...
::  如果在ARM64 Windows上由x64进程启动，则使用ARM32进程重新启动脚本
...
%eline%
echo 脚本是从临时文件夹启动的，
echo 很可能是您直接从压缩包中运行脚本。
echo:
echo 请解压压缩包并从解压后的文件夹中启动脚本。
goto done2
)
)
...
echo:
echo PowerShell无法正常工作。正在中止...
echo 如果您对PowerShell应用了限制，请撤销这些更改。
echo:
echo 请查看此页面以获取帮助。%mas%IAS-Help#troubleshoot
goto done2
)
...
::  禁用快速编辑并从conhost.exe启动以避免终端应用程序
...
::  以下代码也适用于ARM64 Windows 10（包括x64位模拟）
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "由 Astro-Saurav 制作"
echo:            ___________________________________________________ 
echo:
echo:              IDM 重置激活工具中文版
echo:               原版 Github: https://github.com/Astro-Saurav
echo:            ___________________________________________________ 
echo:                                                               
echo:               [1] 激活
echo:               [2] 冻结试用（重新试用30天）
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "请在键盘上输入菜单选项 [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
echo 正在 %SystemRoot%\Temp 中创建 CLSID 注册表项的备份
...
echo %line%
echo:
echo 激活对某些用户可能不起作用，IDM 可能会显示假的序列号提醒界面。
echo:
...
::  使用 ping 和端口 80 测试检查 internetdownloadmanager.com 的互联网连接
...
call :_color %Red% "无法连接 internetdownloadmanager.com，正在中止..."
goto done
)
call :_color %Gray% "Ping 命令对 internetdownloadmanager.com 失败"
echo:
)
...
echo 正在检查信息 - [%regwinos% ^| %fullbuild% ^| %regarch% ^| IDM: %IDMver%]
...
echo:
call :_color %Gray% "如果出现假序列号界面，请改用冻结试用选项。"
) else (
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "正在搜索 $regPath 中的 IDM CLSID 注册表项"
	Write-Host
    ...
    foreach ($key in $filteredKeys) {
        ...
        if (($defaultValue -match "^\d+$") -and ($key.SubKeyCount -eq 0)) {
            ...
            Write-Output "$($key.PSChildName) - 在默认值中发现数字且无子项"
            continue
        }
        if (($defaultValue -match "\+|=") -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在默认值中发现 + 或 = 且无子项"
            continue
        }
        ...
        if (($versionValue -match "^\d+$") -and ($key.SubKeyCount -eq 1)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在 \Version 中发现数字且无其他子项"
            continue
        }
        $keyValues.PSObject.Properties | ForEach-Object {
            if ($_.Name -match "MData|Model|scansk|Therad") {
                $finalValues += $($key.PSChildName)
                Write-Output "$($key.PSChildName) - 发现 MData Model scansk Therad"
                continue
            }
        }
        if (($key.ValueCount -eq 0) -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 发现空项"
            continue
        }
    }
}
...
if ($finalValues -ne $null) {
    Write-Host
    if ($lockKey -ne $null) {
        Write-Host "正在锁定 IDM CLSID 注册表项..."
    }
    if ($deleteKey -ne $null) {
        Write-Host "正在删除 IDM CLSID 注册表项..."
    }
    Write-Host
} else {
    Write-Host "未找到 IDM CLSID 注册表项。"
	Exit
}
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 项数量超过 20 个。现在将其删除而不是锁定..."
	Write-Host
}
...
function Take-Permissions {
    ...
    $TypeBuilder.DefinePInvokeMethod('RtlAdjustPrivilege', 'ntdll.dll', 'Public, Static', 
